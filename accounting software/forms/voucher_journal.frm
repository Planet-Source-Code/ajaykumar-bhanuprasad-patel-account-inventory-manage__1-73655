...
Begin VB.Form vchr_Journal 
   ...
   End
End
Attribute VB_Name = "vchr_Journal"
...
If delete_sure = 6 Then
    ...
    selected_procedure = "journal voucher"
    xsub_entry_no = 0
...
Do Until rs_acn_tran_jrn.EOF
    If rs_acn_tran_jrn!fin_acnt_trn_vcno = Text1.Text Then
        rs_acn_tran_jrn.Delete
        rs_acn_tran_jrn.UpdateBatch
...
Do Until rs_acn_tran_all.EOF
    If rs_acn_tran_all!fin_acnt_trn_vcno = Text1.Text And rs_acn_tran_all!fin_acnt_trn_vchr = "journal" Then
...
Do Until rs_acn_tran_all_temp.EOF
    If rs_acn_tran_all_temp!fin_acnt_trn_vcno = Text1.Text And rs_acn_tran_all_temp!fin_acnt_trn_vchr = "journal" Then
...
If Text1.Text = "" Or _
    ...
    DTPicker1.Value < this_year_starting_date Then
...
list_lgr.Top = -340
selected_procedure = "Adjustments/Journal voucher"
cmd_delete.Enabled = False
...
If rs_acn_tran_jrn.RecordCount = 0 Then Text1.Text = 1
Text4.Text = ""
...
Call open_rs_acn_tran_jrn
Do Until rs_acn_tran_jrn.EOF
If rs_acn_tran_jrn!fin_acnt_trn_vcno = selected_voucher_no Then
...
If rs_acn_tran_jrn.RecordCount > 0 Then
    For iflvn = 1 To rs_acn_tran_jrn.RecordCount
    this_voucher_no = rs_acn_tran_jrn!fin_acnt_trn_vcno
...
For transaction_counter = 1 To sub_entry_no
    ...
    Call open_database
    Call open_rs_acn_tran_jrn
    For available_tran_no = 1 To rs_acn_tran_jrn.RecordCount
    With rs_acn_tran_jrn
    ...
    If !fin_acnt_trn_vcno = Text1.Text And !fin_acnt_trn_seno = transaction_counter Then
        ...
        If LCase(combo_sub_entry_no(transaction_counter).Text) = LCase("Dr / By") Then
        !fin_acnt_trn_side = "dr"
        ElseIf LCase(combo_sub_entry_no(transaction_counter).Text) = LCase("Cr / To") Then
        !fin_acnt_trn_side = "cr"
        End If
        !fin_acnt_trn_nrtn = Text4.Text
        !fin_acnt_trn_user = Text5.Text
        !fin_acnt_trn_vchr = "journal"
...
Dim this_v_no
Dim last_v_no
this_v_no = !fin_acnt_trn_vcno
If last_v_no <> this_v_no Then
...
Call refresh_dr_cr_total_amt
    ...
    End If
        ...
        If voucher_total_dr_amt < voucher_total_cr_amt Then
            ...
            If LCase(combo_sub_entry_no(Index).Text) = LCase("Dr / By") And Index > 1 And Index = sub_entry_no Then
                    sub_entry_no = sub_entry_no + 1
                    total_sub_entry_no = total_sub_entry_no + 1
                    Load combo_sub_entry_no(sub_entry_no)
                    Load combo_lgr(sub_entry_no)
...
For int_i = 1 To total_sub_entry_no
    If LCase(combo_sub_entry_no(int_i).Text) = LCase("Cr / To") Then
        voucher_total_cr_amt = voucher_total_cr_amt + Val(text_amt(int_i).Text)
    ElseIf LCase(combo_sub_entry_no(int_i).Text) = LCase("Dr / By") Then
...
ElseIf LCase(combo_sub_entry_no(Index).Text) = LCase("Cr / To") Then
            this_entry = "cr"
                    'write here text to insert row
                    Call add_account_combo_lgr(sub_entry_no)
                    Call cr_amt_text_adjust(sub_entry_no)
...
Call unlock_all_combo_text
End Sub
Private Sub combo_lgr_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
If KeyCode = 13 Then
    Call open_database
    Call open_rs_lgr_main_dtl
    Do Until rs_lgr_main_dtl.EOF
    If combo_lgr(Index).Text = rs_lgr_main_dtl!lgr_main_dtl_alis Then
        combo_lgr(Index).Text = rs_lgr_main_dtl!lgr_main_dtl_name
    End If
    rs_lgr_main_dtl.MoveNext
    Loop
Dim lgr_is_available_or_not As Integer
...
Call open_rs_lgr_main_dtl
Do Until rs_lgr_main_dtl.EOF
    If combo_lgr(Index).Text = rs_lgr_main_dtl!lgr_main_dtl_name Then
    lgr_is_available_or_not = 1
    Else
    End If
rs_lgr_main_dtl.MoveNext
...
If lgr_is_available_or_not = 0 Then
    MsgBox "You are Entered invalid ledger...!!! select proper account...!!!"
    combo_lgr(Index).Text = "select a ledger"
    combo_lgr(Index).SetFocus
    Exit Sub
End If
text_amt(Index).SetFocus
End If
End Sub
